...
class UserController extends BaseController
{
    ...
    */
    public function serList()
    {
        ...
        if(!$ServiceType && !$ServiceArea) {
            ...
            $services = Service::where('ServiceArea','like','%'.$ServiceArea.'%')->orderBy('updated_at','desc')->lists('ServiceID');
            $counts = DB::table('T_P_SERVICECERTIFY')->where('T_P_SERVICECERTIFY.State','1')->whereIn('ServiceID',$services)->count();
            $services = DB::table('T_P_SERVICECERTIFY')->skip($skipnum)->take($pagecount)->where('T_P_SERVICECERTIFY.State','1')->whereIn('ServiceID',$services)->orderBy('updated_at','desc')->lists('ServiceID');
            $pages = ceil($counts/$pagecount);
        } elseif ($ServiceType && $ServiceArea) {
            $services1 = Service::where('ServiceArea','like','%'.$ServiceArea.'%')->orderBy('updated_at','desc')->lists('ServiceID')->toArray();
    ...
    */
    ...
    {   
        ...
        try {
            ...
            User::where('userid', $UserID)->update(['UserPicture'=>$payload['UserPicture']]);
            DB::commit();
        } catch (Exception $e){
            DB::rollback();
            throw $e;
    ...
    */
    ...
    public function cooList() {
        ...
        $UserID = $this->auth->user()->toArray()['userid'];
        $projects = Project::where(['PublishState' => 1, "ServiceID" => $UserID])->orWhere(['PublishState' => 1, "UserID" => $UserID])->orderBy('updated_at','desc')->lists('ProjectID');
        $counts = count($projects);
        $pages = ceil($counts/$pagecount);

        $projects = Project::skip($skipnum)->take($pagecount)->where(['PublishState' => 1, "ServiceID" => $UserID])->orWhere(['PublishState' => 1, "UserID" => $UserID])->orderBy('updated_at','desc')->lists('ProjectID');
    ...
    public function rushList() {
        ...
        $Project = new ProjectController();
        $UserID = $this->auth->user()->toArray()['userid'];
        $projects = DB::table('T_P_RUSHPROJECT')->where('ServiceID',$UserID)->orderBy('updated_at','desc')->lists('ProjectID');
        $counts = count($projects);
        $pages = ceil($counts/$pagecount);

        $projects = DB::table('T_P_RUSHPROJECT')->skip($skipnum)->take($pagecount)->where("ServiceID", $UserID)->orderBy('updated_at','desc')->lists('ProjectID');
